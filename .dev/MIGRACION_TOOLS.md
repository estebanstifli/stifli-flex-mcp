# Migración de herramientas MCP a StifliFlexMcp

Este documento describe los pasos para migrar todas las herramientas (tools) del modelo MCP original (`WaicMcpModel`) al nuevo plugin `StifliFlexMcpModel`, asegurando compatibilidad total.

## Pasos de migración

1. **Preparar el entorno y helpers**
   - Verificar que existen y funcionan los helpers/utilidades: `StifliFlexMcpUtils`, `StifliFlexMcpFrame`, `StifliFlexMcpDispatcher`.
   - Confirmar que el modelo base y el método `dispatchTool` están definidos.

2. **Copiar el array completo de herramientas**
   - Copiar el array `$tools` de `WaicMcpModel::getTools()` a `StifliFlexMcpModel::getTools()`, adaptando nombres y dependencias si es necesario.
...
4. **Adaptar dependencias y comprobaciones de permisos**
   - Cambiar `WaicUtils` por `StifliFlexMcpUtils`, `WaicFrame` por `StifliFlexMcpFrame`, etc.
